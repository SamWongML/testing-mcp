...
**Current state:** P0–P3 done; P4 (compile + CLI + sample corpus) in progress. `@atp/schema` and
`@atp/engine` are implemented; `reporting`, `store`, `mcp-server`, `cli`, and `tools/compile` are
...
pnpm compile                 # discovery → dist/manifest.json   (stub until P4)
```
