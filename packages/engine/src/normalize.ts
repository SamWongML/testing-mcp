...
/**
 * Normalize an authored test/suite into serializable manifest entries ( * ). This is the compile step's core transform: it strips every function
 ...
 * edges throw here, the compile-time check); a matrix expands into one entry per
 * cartesian cell, each carrying its resolved env and cell coordinates.
 */

/** Above this authored `timeoutMs`, a run is treated as long-running (→ MCP Task by
 * default) unless the author sets `isLongRunning` explicitly. */
